...
describe('Forms Unit Tests', () => {
    describe('Fetch Form Metadata and associated Member Data', () => {
        const memberData = {
            ...
            termComments:
                'The documentation was very easy to follow and well put together.',
        ...
        it('Test No. 1', async () => {
            ...
            const { _id: nameSectionId } = await FormSection.findOne({
                name: 'fullName',
            });
            const { _id: phoneSectionId } = await FormSection.findOne({
                name: 'phoneNumber',
            });
            const { _id: nextTermActivitySectionId } =
                await FormSection.findOne({
                    name: 'nextTermActivity',
                });
            const { _id: nextTermRoleSectionId } = await FormSection.findOne({
                name: 'nextTermRole',
            });
            const formData = {
            ...
            const { form, user } = await data.forms.fetchFormAndMemberData(
                userId,
                formName
            );
        ...
        it('Test No. 2', async () => {
            ...
            const { _id: personalEmailSectionId } = await FormSection.findOne({
                name: 'personalEmail',
            });
            const { _id: studentIdSectionId } = await FormSection.findOne({
                name: 'studentId',
            });
            const { _id: termStatusSectionId } = await FormSection.findOne({
                name: 'termStatus',
            });
            const { _id: subteamsSectionId } = await FormSection.findOne({
                name: 'subteams',
            });
            const { _id: designCentreSafetySectionId } =
                await FormSection.findOne({
                    name: 'designCentreSafety',
                });
            ...
            const formData = {
                ...
                description:
                    'Registration Form - A short blurb to describe this form',
                name: 'register',
                sections: [
                    {
                        section: nameSectionId,
    ...
    describe('Update Form Sections', () => {
        it('Test Update Success', async () => {
            ...
            const fetchedFormSections = (await FormSection.find().lean()).map(
                (section) => _.omit(section, '_id')
            ); // Strip away the _id property
    ...
    describe('Create New Form', () => {
        it('Create Success - Update Existing Form Sections', async () => {
            ...
            const formData = {
                title: 'Sign Up Form',
                description:
                    'Sign Up Form - A short blurb to describe this form',
                name: 'signup',
                sections: [
                    {
                        name: 'fullName',
            ...
            const fetchedFormSections = (await FormSection.find().lean()).map(
                (section) => _.omit(section, '_id', '__v')
            ); // Strip away the _id property
            const fetchedFormData = await Form.findOne({
                name: 'signup',
            }).lean();
            expect(res.statusCode).toBe(200);
    ...
    describe('Update Form Data', () => {
        it('Update Success - Update Existing Form Sections', async () => {
            ...
            await data.forms.updateFormMetadata(
                formName.toString(),
                updatedFormData,
                res
            );
            ...
            const subteamSectionId = fetchedFormSections
                .find((s) => s.name === 'subteams')
                ._id.toString();
            const designCentreSafetySectionId = fetchedFormSections
                .find((s) => s.name === 'designCentreSafety')
                ._id.toString();
            const nameSectionId_Final = fetchedFormSections
                .find((s) => s.name === 'fullName')
                ._id.toString();

            fetchedFormSections = fetchedFormSections.map((section) =>
                _.omit(section, '_id', '__v')
            ); // Strip away the _id property
            let fetchedFormData = await Form.findOne({ name: 'bot' }).lean();
            fetchedFormData.sections = fetchedFormData.sections.map((s) => {
                return {
                    ...s,
