            ...
            JSON.stringify(
                await data.util.resWrapper(async () => {
                    ...
                    if (payload.subteams.length === 0) {
                        ...
                        relevantTaskIDs = intersection(
                            ...(
                                await data.subteams.search({}, { tasks: 1 })
                            ).map((st) =>
                                st.tasks.map((taskID) => taskID.toString())
                            )
                        );
                    } else {
                        // If subteam(s) are specified, then choose only the tasks that are relevant to the chosen subteams
                        relevantTaskIDs = union(
                            ...(
                                await data.subteams.search(
                                    { name: { $in: payload.subteams } },
                                    { tasks: 1 }
                                )
                            ).map((st) =>
                                st.tasks.map((taskID) => taskID.toString())
                    ...
                    const pendingTasks = relevantTaskIDs.map((taskId) => {
                        return { taskId: taskId, status: 'pending' };
                    });

                    // Assign "irrelevant" status to the remaining tasks
                    const allTaskIDs = (await data.task.getAll()).map(
                        (task) => task.id
                    );
                    const irrelevantTaskIDs = allTaskIDs.filter(
                        (taskID) => !relevantTaskIDs.includes(taskID)
                    );
                    const irrelevantTasks = irrelevantTaskIDs.map((taskId) => {
                        return { taskId: taskId, status: 'irrelevant' };
                    });
