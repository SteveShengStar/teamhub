...
import {
    ...
    getFilters,
    DataFetchType,
} from '../frontend/store/reducers/membersReducer';
...
const Home = () => {
    ...
    const {
        ...
        fetchingData,
        fetchedMembers,
    } = useSelector((state) => state.membersState);
    ...
    useEffect(() => {
            ...
            getFilters(dispatch, router).then((success) => {
                if (success)
                    searchMembers(
                        ...
                        {
                            subteams: {
                                $in: user.subteams.map((st) => st._id),
                            },
                        },
                        router
                    );
        ...
        hydrated,
        user,
    ]); /* hydrated is set to true once data is re-loaded into the Redux store after a page-refresh/page-reload */
    ...
    const getActionText = () => {
        if (activeAction === 'slack') return 'Message';
        else if (activeAction === 'email') return 'Email';
        return '';
    };

    const onExecuteAction = () => {
        if (activeAction === 'email') generateGroupEmail();
    };
        ...
        <PageTemplate title='Explore'>
            ...
            >
                ...
                >
                    ...
                    >
                        <Header3 style={{ transformOrigin: 'left' }}>
                            Members
                        </Header3>
                        ...
                        <SystemComponent>
                            ...
                            >
                                <OptionButton
                                    onClick={() => toggleGroupAction('email')}
                                    isSelected={activeAction === 'email'}
                                >
                                    Email
                                </OptionButton>
                            ...
                            {activeAction && (
                                <SystemComponent textAlign='right'>
                                    Select a bunch of members below, then Click
                                    Send {getActionText()}.
                                </SystemComponent>
                            )}
                        </SystemComponent>
...
const MembersListCard = styled(Card)`
    ...
    width: calc(
        100vw -
            ${(props) =>
                2 * props.theme.space.cardMarginSmall +
                2 * props.theme.space.cardPaddingSmall}px
    );
    top: 0;
...
const OptionButton = styled(Button)`
    ...
    ${(props) =>
        props.isSelected && '&:hover {transform: scale(1); pointer: default;}'}
