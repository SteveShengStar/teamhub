...
export const userLogout = async (userId, dispatch, router) => {
    ...
    if (res && res.success) {
        ...
        console.error(
            'Received a 403 return code. The user login token must have expired.'
        );
    }
};
 ...
 * @param {string} id
 */
export const updateUser = async (dispatch, options, id, router) => {
    try {
        ...
        throw new Error(
            'userReducer.js#updateUser(): Failed to retrieve user data.'
        );
    } catch (err) {
        throw new Error(err);
    }
 ...
 * @param {string} id
 */
export const updateProfileInfo = async (dispatch, options, id, router) => {
    try {
        const res = await api.members.update(options, id, dispatch, router);
        if (res && res.success) {
