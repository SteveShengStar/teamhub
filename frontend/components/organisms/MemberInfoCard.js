...
const MemberInfoCard = ({ memberData, className, onClose, animRef }) => {
    const memberSubteams =
        memberData.subteams && memberData.subteams.length > 0
            ? memberData.subteams.map((st) => st.name)
            : [];
    const memberProjects =
        memberData.projects && memberData.projects.length > 0
            ? memberData.projects.map((prj) => prj.name)
            : [];
    const subteam =
        memberSubteams && memberSubteams.length > 0 ? memberSubteams[0] : '';
    ...
    return (
        <InfoCard className={className} ref={animRef}>
            ...
            >
                ...
                <Button alignSelf='start' justifySelf='end' onClick={onClose}>
                    Close
                </Button>
            </SystemComponent>
            ...
            <ContentContainer>
                <LeftColumn>
                    <Header2 fontSize='smallTitle'>
                        {memberData.name
                            ? memberData.name.first + ' ' + memberData.name.last
                            : ''}
                    </Header2>
                    {memberData.subteams ? (
                        <MemberSubtitle>
                            {(memberData &&
                                ...
                                memberData.memberType.name) ||
                                'Member'}{' '}
                            on{' '}
                            <BorderlessButton
                                variant={
                                    (subteam && subteam.toLowerCase()) || ''
                                }
                                fontSize='inherit'
                                fontWeight='inherit'
                            >
                    ...
                    {memberSubteams && memberSubteams.length > 0 && (
                        <>
                            ...
                            <SubteamsContainer>
                                {memberSubteams.map((subteam, i) => (
                                    <Button
                                        key={i}
                                        variant={subteam.toLowerCase()}
                                    >
                                        {subteam}
                                    </Button>
                                ))}
                ...
                <RightColumn>
                    <PersonalCard mb={3}>
                        <Image
                            src={
                                (memberData.imageUrl &&
                                    memberData.imageUrl.replace(
                                        /s96/,
                                        's400'
                                    )) ||
                                '/static/default-headshot.png'
                            }
                            width={'100%'}
                        />
                        <SystemComponent
                        ...
                        >
                            <InlineItemRow>
                                ...
                                <Link
                                    ...
                                    href={
                                        memberData.email
                                            ? `https://mail.google.com/mail/?view=cm&fs=1&tf=1&to=${memberData.email}`
                                            : ''
                                    }
                                >
                                    Email
