...
const EditLinksModal = ({ visible, handleCloseModal }) => {
    ...
    const facebookUrl =
        user.links && user.links.find((l) => l.type === 'facebook')
            ? user.links.find((l) => l.type === 'facebook').link
            : '';
    const linkedInUrl =
        user.links && user.links.find((l) => l.type === 'linkedin')
            ? user.links.find((l) => l.type === 'linkedin').link
            : '';
    const githubUrl =
        user.links && user.links.find((l) => l.type === 'github')
            ? user.links.find((l) => l.type === 'github').link
            : '';
    const websiteUrl =
        user.links && user.links.find((l) => l.type === 'website')
    ...
    const handleSave = (evt) => {
        ...
        validateUrl(errors, 'githubUrl', githubUrl, [
            'www.github.com',
            'github.com',
        ]);
        validateUrl(errors, 'linkedInUrl', linkedInUrl, [
            'www.linkedin.com',
            'linkedin.com',
        ]);
        ...
        if (dataIsValid) {
            facebookUrl =
                !facebookUrl || facebookUrl.slice(0, 4) === 'http'
                    ? facebookUrl
                    : 'https://'.concat(facebookUrl);
            githubUrl =
                !githubUrl || githubUrl.slice(0, 4) === 'http'
                    ? githubUrl
                    : 'https://'.concat(githubUrl);
            linkedInUrl =
                !linkedInUrl || linkedInUrl.slice(0, 4) === 'http'
                    ? linkedInUrl
                    : 'https://'.concat(linkedInUrl);
            websiteUrl =
                !websiteUrl || websiteUrl.slice(0, 4) === 'http'
                    ? websiteUrl
                    : 'https://'.concat(websiteUrl);
                ...
                >
                    ...
                    <SystemComponent>
                        <URLField
                            ...
                            errorText={
                                'Please Enter Valid Facebook Profile Url.'
                            }
                            onHandleChange={handleChange}
                        />
                    </SystemComponent>
