...
const OptionRow = ({
    ...
    return (
        <>
            ...
            >
                <SystemSpan
                    color={readOnly && `${theme.colors.greys[3]}`}
            ...
            {!readOnly && (
                <RemoveOptionButton>
                    <span
                        className='fa fa-solid fa-xmark fa-lg'
                        onClick={() => handleOptionDelete(optionIdx)}
                    />
                </RemoveOptionButton>
            )}
        </>
...
const AddOptionButton = ({ handleOptionAdd }) => {
    return (
        <>
            <SystemComponent />
            <GhostButton variant='neutral' onClick={() => handleOptionAdd()}>
                Add Option
            </GhostButton>
        </>
    );
...
const OptionsList = ({
    ...
    if (type === 'boolean') {
        ...
        return (
            <>
                <SystemComponent
                    fontSize={`${theme.fontSizes.body2}px`}
                    gridColumn='1/4'
                >
                    Edit Your Options Below:
                </SystemComponent>
    ...
    return (
        <BaseSection
            ...
            handleSelectModifier={(modifier, selected) => {
                ...
                if (idx === -1) {
                    throw new Error(
                        'Selectable.js: Could not find the appropriate section modifier.'
                    );
                }
        ...
        >
            <SystemComponent
                display='grid'
                gridTemplateColumns='20px auto 12px'
                gridColumnGap={`${theme.space[3]}px`}
                gridRowGap={`${theme.space[3]}px`}
            >
                <OptionsList
                    options={options}
                    type={type}
                    handleOptionAdd={() => handleOptionAdd(sectionName)}
                    handleOptionChange={(optionIdx, newValue) =>
                        handleOptionChange(sectionName, optionIdx, newValue)
                    }
                    handleOptionDelete={(optionIdx) =>
                        handleOptionDelete(sectionName, optionIdx)
                    }
                />
            </SystemComponent>
        </BaseSection>
