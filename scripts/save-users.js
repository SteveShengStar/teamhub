...
async function main() {
  ...
  try {
    ...
    await Promise.all(parsedMembers.map(member => {
      const foundMember = members.find(m => m.name.display === member.name);
      const desiredSubteam = subteams.find(subteam => subteam.name === member.team)
      const desiredType = memberTypes.find(memberType => memberType.name === member.position);

      if (!foundMember) {
        const nameChunks = member.name.split(' ');
        const first = nameChunks[0].toLowerCase();
        const last = nameChunks[nameChunks.length - 1].toLowerCase();
      ...
      if (desiredSubteam && desiredType) {
        return data.members.updateMember({_id: foundMember._id}, {
          subteams: [desiredSubteam.name],
          memberType: desiredType.name
        })
      }
